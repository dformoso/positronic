...
- **Defining** — fuzzy goal, no spec yet → run `define`. Once define converges:
  ...
  - **Custom LLM harness on the table.** When the system needs to run reliably without a human in the verification loop — programmatic gates, contracts, audit, autonomous execution beyond conversational — off-the-shelf coding agents stop being enough. After `/to-prd`, `define` hands off to `pick-harness-shape`, which first decides whether a custom harness is needed and then walks substrate, topology, memory, tools, and gates, writing a versioned `harness/` artifact that `/to-spec` reads. **Exception:** when the harness IS the product or differentiator, invoke `pick-harness-shape` *before* `/to-prd` so its picks can shape the PRD.
  - **UI product on the table.** When the product has persistent user-facing surfaces (web app, mobile, dashboard, browser extension), the structural shape of those surfaces — nav, onboarding, settings, account lifecycle, error states — needs locking down before SPEC. After `/to-prd`, `define` hands off to `pick-surfaces`, which walks the load-bearing UX decisions and writes a versioned `surfaces/` artifact that `/to-spec` reads. If both this and `pick-harness-shape` apply, run `pick-surfaces` first so harness picks can slot into known surfaces.
