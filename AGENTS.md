...
- **Defining** — a fuzzy goal with no spec yet, *or a scoped change to an existing product when a PRD/spec already exists* → run `define`. Don't silently start coding a feature request on an existing product — name the phase first. Once define converges:
  ...
  - **Zero-to-one with market uncertainty.** Prompt the user to run `/research-market` (forum + competitive evidence), `/ideate` (10 ranked one-pagers, user picks), `/judge-idea` (adversarial gate), then `/to-prd`, `/to-frd`, `/to-spec`, `/to-issues`.
  - **Established space.** Prompt the user to run `/to-prd`, `/to-frd`, `/to-spec`, `/to-issues` directly.
  - **How far a change travels.** The PRD's **change tier** decides which artifacts a change touches at all: `fix` (none — straight to the code), `internal` (SPEC only), `feature` (PRD → FRD → surfaces + mockup → SPEC → issues), `launch` (all of that, plus a cutover runbook, `/go-live`, and a `/readout` date). Name the tier out loud; applying the heaviest one to everything is the failure mode of a framework like this. Ladder in `docs/amend-mode.md`.
  - **The brief-PRD / detailed-FRD split.** `/to-prd` stays under two pages so a person actually re-reads it: problem, user, journeys, metrics, kill criteria, scope. `/to-frd` carries everything that scales — per-feature states, rules, edge cases, and Given/When/Then acceptance criteria that `/to-issues` copies into issues and `test-driven-dev` turns into the red test. Don't let functional detail creep back into the PRD.
  ...
  - **UI product on the table.** When the product has persistent user-facing surfaces (web app, mobile, dashboard, browser extension), the shape of those surfaces — visual identity (feel, signature, accent, type) plus structure (nav, onboarding, settings, account lifecycle, error states) — needs locking down before SPEC. After `/to-frd`, `define` hands off to `pick-ui-surfaces`, which walks the load-bearing decisions and writes `definitions/surfaces.md`, which `/to-spec` reads. If both this and `pick-harness-shape` apply, run `pick-ui-surfaces` first so harness picks can slot into known surfaces. Then prompt `/to-mockup`: it renders every FRD state and every system state as annotated screens in one self-contained `definitions/mockup.html` — the first point at which anyone can argue with the product instead of with a table. Its findings amend the FRD and surfaces; the chain is a loop, not a waterfall.
