#!/usr/bin/env bash
# Freshness sweep over dated decision records (docs/adr/, definitions/runtime/).
...
today=$(date +%F)
grep -rn 'NEXT REVIEW:' docs/adr definitions/runtime 2>/dev/null | while IFS= read -r hit; do
...
grep -rnE '^[-*[:space:]]*TRIGGER:' docs/adr definitions/runtime 2>/dev/null | sed 's/^/TRIGGER   /'
exit 0
