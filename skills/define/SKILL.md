---
name: define
description: Defining-phase orchestrator. Surfaces assumptions, frames a falsifiable hypothesis, then routes to the right next step (research-market / ideate / judge-idea / to-prd / to-frd / pick-ui-surfaces / pick-harness-shape / to-mockup). Use when the user has a fuzzy product or feature idea — whether greenfield (no PRD/SPEC yet) or a scoped change (increment) to an existing product.
---
...
Either way `/to-prd` is followed by `/to-frd`, which expands each feature's behaviour, states, and acceptance criteria into `definitions/frd.md`. Everything downstream reads that file, so name it as the next step rather than leaving it implicit — a `fix`- or `internal`-tier change is the only case that skips it.

**Step 7 — Hand off to pick-ui-surfaces.** If the product has persistent UI surfaces (web app, mobile, dashboard, browser extension), invoke pick-ui-surfaces *after* `/to-frd` has run, so the UI picks — visual identity plus structure — ground on the PRD's user and the FRD's feature set, channels, and integrations. It writes `definitions/surfaces.md`, which `/to-spec` reads downstream. Skip for CLI, API, library, or headless products.

**Step 8 — Hand off to pick-harness-shape.** If the system needs to run reliably without a human in the verification loop — programmatic gates, contracts, audit, autonomous execution — or falls into a non-coding domain (support, ops, research, drafting), invoke pick-harness-shape *after* `/to-prd` (and after Step 7, if it applied — harness picks then know which UI surfaces they're slotting into), so the harness picks ground on the PRD's user, regulatory, and business constraints. It writes `definitions/harness.md`, which `/to-spec` reads downstream, so this is not optional when a custom harness is needed. (Exception: if the harness IS the product or differentiator, picks may shape the PRD rather than follow from it — pick-harness-shape will surface this and offer the override.)

**Step 9 — Prompt the mockup, then the SPEC.** For a UI product, prompt the user to run `/to-mockup` once surfaces (and harness, if any) are locked. It renders every FRD state and every system state as real screens in one annotated `definitions/mockup.html` — the first point at which anyone can disagree with the product rather than with a table, and cheap enough to be worth arguing over. Its findings amend the FRD and surfaces in the normal way; the chain is a loop, not a waterfall. Then `/to-spec`.
