...
**Step 7 — Hand off to pick-ui-surfaces.** If the product has persistent UI surfaces (web app, mobile, dashboard, browser extension), invoke pick-ui-surfaces *after* `/to-prd` has run, so the UI picks — visual identity plus structure — ground on the PRD's user, form factor, and constraints. It writes `definitions/surfaces.md`, which `/to-spec` reads downstream. Skip for CLI, API, library, or headless products.
