...
**Step 7 — Hand off to pick-ui-surfaces.** If the product has persistent UI surfaces (web app, mobile, dashboard, browser extension), invoke pick-ui-surfaces *after* `/to-prd` has run, so the UI picks — visual identity plus structure — ground on the PRD's user, form factor, and constraints. It writes a versioned `definitions/surfaces/<file>.md` that `/to-spec` reads downstream. Skip for CLI, API, library, or headless products.

**Step 8 — Hand off to pick-harness-shape.** If the system needs to run reliably without a human in the verification loop — programmatic gates, contracts, audit, autonomous execution — or falls into a non-coding domain (support, ops, research, drafting), invoke pick-harness-shape *after* `/to-prd` (and after Step 7, if it applied — harness picks then know which UI surfaces they're slotting into), so the harness picks ground on the PRD's user, regulatory, and business constraints. It writes a versioned `definitions/harness/<file>.md` that `/to-spec` reads downstream, so this is not optional when a custom harness is needed. (Exception: if the harness IS the product or differentiator, picks may shape the PRD rather than follow from it — pick-harness-shape will surface this and offer the override.)
