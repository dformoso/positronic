...
**Step 7 — Hand off to pick-harness-shape.** If the project involves a custom LLM/agent harness — non-coding agent work, multi-agent workflow, or any system where Claude Code / off-the-shelf doesn't fit — invoke pick-harness-shape *after* `/to-prd` has run, so the harness picks ground on the PRD's user, regulatory, and business constraints. It writes a versioned `harness/<file>.md` that `/to-spec` reads downstream, so this is not optional when the harness matters. (Exception: if the harness IS the product or differentiator, picks may shape the PRD rather than follow from it — pick-harness-shape will surface this and offer the override.)
