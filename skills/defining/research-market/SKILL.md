...
2. **Build a source list.** Curated starting points; adjust per project type:
   - Reddit (search via `site:reddit.com` web search)
   - Hacker News (Algolia search, unauth JSON)
   - Indie Hackers
   - Product Hunt
   - App Store / Play Store reviews (via web search)
   - G2 / Capterra (B2B)
   - Domain forums and Discord recaps

   Aim for ≥3 distinct platforms.
