...
10. **HITL bash script.** Last resort. If a human must click, drive _them_ with `${SKILL_DIR}/scripts/hitl-loop.template.sh` (`${SKILL_DIR}` = the directory containing this file) so the loop is still structured. Captured output feeds back to you.
...
**Audit the existing tests.** A green suite that shipped this bug is itself a bug. Open the closest existing test, find why it missed (wrong call shape / fixture / assertion — see [`tests.md`](../test-driven-dev/tests.md)), and patch it. A new regression test alone isn't enough.
...
**And ask what delayed the diagnosis.** If the answer is obscurity rather than structure — a name that lied, a missing invariant comment, a hidden dependency discovered the hard way — move that understanding into the code now, while you hold it: a rename, an interface comment, a guard clause, in its own refactor-only commit after the fix. Bigger comprehension debt goes to `improve-readability` with the confusion noted. The bug you just chased is the freshest map of where the code misleads its readers.

**And check what the fix left behind.** If it added a third guard on an invariant two other places already check, or another `catch` on a path that can't fail, the fix treated a symptom of excess machinery — hand it to `improve-readability` (excess-machinery lens) rather than letting the guards accrete.
