...
# ── test 5: /test-driven-dev prompt includes issue number, title, and criteria ────────────
...
grep -q '/test-driven-dev' "$prompt_log" \
  && ok "prompt uses /test-driven-dev" || fail "prompt uses /test-driven-dev"
