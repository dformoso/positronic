...
For each issue in the wave, fetch the full body, then build a `/tdd` prompt that includes the full issue body and pointers to the SPEC and PRD (when present). Redirect each subprocess's output to a per-issue log file so per-issue results can be surfaced after `wait`:
...
for num in "${wave[@]}"; do
  ...
  prompt="/tdd Implement GitHub issue #${num}: ${title}
